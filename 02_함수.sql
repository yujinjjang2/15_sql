...
SELECT 
	ABS( CEIL( MONTHS_BETWEEN(SYSDATE, TO_DATE('2023-12-22', 'YYYY-MM-DD'))) ) "수강 기간(개월)",
	CEIL( MONTHS_BETWEEN(TO_DATE('2023-12-22', 'YYYY-MM-DD'), SYSDATE)) 
FROM DUAL; -- 4.568
...
SELECT TO_CHAR(1000001999, 'FM999,999,999,999') || '원' FROM DUAL;
...
--SELECT EXTRACT(YEAR FROM SYSDATE) || '년 ' || EXTRACT(MONTH FROM SYSDATE) || '월 ' || EXTRACT(DAY FROM SYSDATE) || '일 ' || TO_CHAR(SYSDATE, '(DY)') FROM DUAL;
...
SELECT SUBSTR(EMP_NO, 1, 2) || '년 ' || SUBSTR(EMP_NO, 3, 2) || '월 ' || SUBSTR(EMP_NO, 5, 2) || '일 ' 생년월일
FROM EMPLOYEE;
...
-- NVL(컬럼명, 컬럼값이 NULL 일때 바꿀 값) : NULL인 컬럼값을 다른 다른값으로 변경
...
SELECT EMP_NAME, BONUS, NVL2(BONUS, 'O', 'X') "보너스 수령"
FROM EMPLOYEE;
...
SELECT EMP_NAME, DECODE (SUBSTR(EMP_NO, 8, 1) , '1', '남성', '2', '여성', '춘자') 성별
FROM EMPLOYEE;
...
-- AVG(숫자가 기록된 컬럼명) : 평균
-- 전 직원 급여 평균
SELECT ROUND(AVG(SALARY)), AVG(SALARY) FROM EMPLOYEE;
...
-- 내 풀이
SELECT COUNT(DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남성')) "남성 사원 수"
FROM EMPLOYEE;
