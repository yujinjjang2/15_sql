...
/*  서브쿼리 유형
    ...
    - 단일행 (단일열) 서브쿼리 : 서브쿼리의 조회 결과 값의 개수가 '1개'일 때 
...
--    서브쿼리의 조회 결과 값의 개수가 '1개'인 서브쿼리
--    단일행 서브쿼리 앞에는 '비교 연산자' 사용 (다중행은 불가!)
--    <, >, <=, >=, =, !=/^=/<>
...
-- EMPLOYEE
--SELECT 
--	E.EMP_ID, E.EMP_NAME, E.JOB_CODE, E.DEPT_CODE, E.SALARY, E.HIRE_DATE
--FROM
--	EMPLOYEE E
...
--SELECT 
--	E.DEPT_CODE, D.DEPT_TITLE, SUM(E.SALARY)
--FROM
--	EMPLOYEE E,
--	DEPARTMENT D
--WHERE 1 = 1
...
/*
    ...
    - EXISTS / NOT EXISTS : 값이 존재하는가? / 존재하지 않는가? (true/false)
    
*/
					
--ANY 연산자는 서브쿼리 결과 중 하나 이상이 조건을 만족하면 참을 반환하고,
--ALL 연산자는 서브쿼리 결과의 모든 값이 조건을 만족할 때 참을 반환
...
--	EMPLOYEE E
--WHERE 1 = 1
--  AND SALARY IN (SELECT MAX(SALARY) FROM EMPLOYEE GROUP BY DEPT_CODE)
--ORDER BY 3;	
--					
...
WHERE 1 = 1
  AND E.DEPT_CODE = D.DEPT_ID(+)
  AND E.JOB_CODE = J.JOB_CODE(+)
  AND E.EMP_ID IN (SELECT DISTINCT MANAGER_ID
						FROM EMPLOYEE
...
--								FROM EMPLOYEE
--								WHERE SUBSTR(TO_CHAR(HIRE_DATE, 'YYYYMMDD'), 1, 1) = '2');
...
-- 서브쿼리(먼저 작성)랑 메인쿼리 나눠서 작성 후 합치기 !! 잘하는 TIP!
