...
--  조인되는 테이블의 각 행들이 모두 매핑된 데이터가 검색되는 방법(곱집합) -> 가능한 경우의 수 다 따짐
--> JOIN 구문을 '잘못 작성'하는 경우 CROSS JOIN의 결과가 조회됨 -> 오류 확인 용도
...
FROM EMPLOYEE E1
LEFT JOIN EMPLOYEE E2 ON(E1.MANAGER_ID = E2.EMP_ID); -- 23행
...
--  AND LOCATION.LOCAL_NAME IN ('ASIA1', 'ASIA2', 'ASIA3');

-- 강사님 풀이
SELECT EMP_ID, EMP_NAME, JOB_NAME, DEPT_TITLE, LOCAL_NAME, SALARY
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE) -- 조인을 언제하든 상관 없음!
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
-- 내 풀이
SELECT
	EMP_NAME, JOB_NAME, DEPT_ID, DEPT_TITLE
FROM
	EMPLOYEE E,
	JOB J,
	DEPARTMENT D
...
SELECT 
	E.EMP_NAME, E.BONUS, D.DEPT_TITLE, D.LOCATION_ID, L.LOCAL_NAME 
FROM 
	EMPLOYEE E,
	DEPARTMENT D,
	LOCATION L
...
SELECT 
	E.EMP_NAME, J.JOB_NAME, L.LOCAL_NAME 
FROM 
	EMPLOYEE E,
	JOB J,
	LOCATION L,
...
-- (SALARY + (SALARY * NVL(BONUS, 0))) * 12 위와 동일

SELECT 
	E.EMP_NAME, J.JOB_NAME, E.SALARY, SALARY * (1 + NVL(BONUS,0) ) * 12 연봉
FROM 
	EMPLOYEE E,
	JOB J,
	SAL_GRADE S
...
SELECT E1.EMP_NAME, E1.DEPT_CODE, E2.DEPT_CODE, E2.EMP_NAME
FROM 
	EMPLOYEE E1,
	EMPLOYEE E2
WHERE 1 = 1
...
-- 내 풀이(틀린 답)
SELECT E1.EMP_NAME, NVL(E1.DEPT_CODE, 0), NVL(E2.EMP_NAME, '-')
FROM EMPLOYEE E1
...
SELECT EMPLOYEE.EMP_NAME, JOB.JOB_NAME, EMPLOYEE.SALARY 
FROM EMPLOYEE 
JOIN JOB ON(EMPLOYEE.JOB_CODE = JOB.JOB_CODE)
